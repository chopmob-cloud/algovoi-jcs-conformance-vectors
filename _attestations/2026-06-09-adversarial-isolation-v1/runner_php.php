<?php
// Generic input runner (PHP / inline JCS RFC 8785). Claim 1 (input bytes) only.
// Usage: php runner_php.php <set.json>
function jcs_canonicalize($v): string {
    ...
    if (is_array($v)) {
        ...
        foreach ($keys as $k) {
            $parts[] = json_encode((string)$k, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR) . ":" . jcs_canonicalize($v[$k]);
        }
...
foreach ($data["vectors"] as $v) {
    if (!array_key_exists("input", $v)) continue;
    $canon = jcs_canonicalize($v["input"]);
    if (base64_encode($canon) === $v["input_jcs_bytes_b64"] && hash("sha256", $canon) === $v["input_content_sha256"]) $p++;

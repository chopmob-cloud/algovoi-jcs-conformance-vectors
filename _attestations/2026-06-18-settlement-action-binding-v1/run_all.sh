#!/usr/bin/env bash
# 8-language byte-for-byte cross-validation — settlement_action_binding_v1.
...
set -u
V="$(cd "$(dirname "$0")/../../vectors/settlement_action_binding_v1" && pwd)/settlement_action_binding_v1.json"
cd "$(dirname "$0")"

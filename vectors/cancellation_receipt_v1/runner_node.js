/**
 * cancellation_receipt_v1 runner (Node.js / @algovoi/substrate).
 *
 * Anchors to IETF draft-hopley-x402-cancellation-receipt-00.
 *
 * Usage:
...
const VECTORS_FILE = join(__dirname, "cancellation_receipt_v1.json");
...
function main() {
  ...
  console.log("cancellation_receipt_v1 runner (Node / @algovoi/substrate)");
  console.log(
  ...
  for (const v of data.vectors) {
    ...
    if (v.receipt) {
      console.log(
        `  ${v.vector_id}: PASS  reason=${v.receipt.cancellation_reason.padEnd(22)} content_hash=${digest.slice(0, 16)}...`,
      );
    } else {
      console.log(

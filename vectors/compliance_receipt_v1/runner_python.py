"""
compliance_receipt_v1 runner (Python).

Validates 8 vectors + 5 pair invariants + 3 chain invariants in
compliance_receipt_v1.json using algovoi-substrate (>=0.3.0) on PyPI.

Anchors to IETF draft-hopley-x402-compliance-receipt-00 (Independent
Submission stream, Informational).
...
VECTORS_FILE = Path(__file__).parent / "compliance_receipt_v1.json"
...
def main() -> int:
    ...
    print("compliance_receipt_v1 runner (Python / algovoi-substrate)")
    print(
    ...
    for vid, v in vectors.items():
        ...
        if "receipt" in v:
            sr = v["receipt"]["screen_result"]
            print(f"  {vid}: PASS  result={sr:6} {label[:14]}={digest[:16]}...")
        else:
            rn = v["row"]["row_number"]
    ...
    total = len(vectors) + len(data["pair_invariants"]) + len(data["chain_invariants"])
    print(

/**
 * compliance_receipt_v1 runner (Node.js / TypeScript reference impl).
 *
 * Validates 8 vectors + 5 pair invariants + 3 chain invariants in
 * compliance_receipt_v1.json using @algovoi/substrate (>=0.3.0).
 *
...
const VECTORS_FILE = join(__dirname, "compliance_receipt_v1.json");
...
function main() {
  ...
  console.log("compliance_receipt_v1 runner (Node / @algovoi/substrate)");
  console.log(
  ...
  for (const v of data.vectors) {
    ...
    if (v.receipt) {
      console.log(
        `  ${v.vector_id}: PASS  result=${v.receipt.screen_result.padEnd(6)} ${label.slice(0, 14)}=${digest.slice(0, 16)}...`,
      );
    } else {
      console.log(

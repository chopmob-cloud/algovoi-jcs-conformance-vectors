<?php
/**
 * epi_interop_v0 runner -- PHP 8.1+ / inline JCS RFC 8785
 *
 * Validates sha256(JCS(input)) == frame_id for each vector
 *
 * Usage (from epi_interop_v0/):  php runner_php.php
 */
...
function jcs_canonicalize($value): string {
    ...
    if (is_float($value)) {
        ...
        $s = json_encode($value, JSON_PRESERVE_ZERO_FRACTION | JSON_THROW_ON_ERROR);
        return $s;
    }
    if (is_string($value)) {
    ...
    if (is_array($value)) {
        ...
        // associative -- sort by UTF-16 code unit order
...
$here = __DIR__;
$data = json_decode(file_get_contents("$here/epi_interop_v0.json"), false, 512, JSON_THROW_ON_ERROR);
$pass = 0; $fail = 0;

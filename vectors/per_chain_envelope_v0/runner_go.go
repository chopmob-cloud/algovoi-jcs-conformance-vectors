// runner_go.go — gowebpki/jcs v1.0.1 runner for x402 per-chain envelope v0.
//
...
type vector struct {
	...
	ExpectedOpenMandateHash   string                 `json:"expected_per_chain_envelope_hash"`
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: go run runner_go.go per_chain_envelope_v0.json")
		os.Exit(2)
	}

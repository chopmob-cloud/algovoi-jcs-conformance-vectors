"""
refund_receipt_v1 runner (Python).

Validates 8 vectors + 5 pair invariants + 3 chain invariants in
refund_receipt_v1.json using algovoi-substrate (>=0.3.0) on PyPI.

Anchors to IETF draft-hopley-x402-refund-receipt-00 (Independent Submission
stream, Informational; AlgoVoi-authored). Composes with
compliance_receipt_v1 via the original_payment_ref linkage.
...
VECTORS_FILE = Path(__file__).parent / "refund_receipt_v1.json"
...
def main() -> int:
    ...
    print("refund_receipt_v1 runner (Python / algovoi-substrate)")
    print(
    ...
    for vid, v in vectors.items():
        ...
        if "receipt" in v:
            expected = v["expected_content_hash"]
            label = "content_hash"
        else:
            expected = v["expected_row_content_hash"]
            label = "row_content_hash"
        ...
        if "receipt" in v:
            rr = v["receipt"]["refund_result"]
            print(f"  {vid}: PASS  result={rr:8} {label[:14]}={digest[:16]}...")
        else:
            rn = v["row"]["row_number"]
    ...
    for ch in data["chain_invariants"]:
        if ch["type"] == "field_equals":
            ...
            if f != ch["value"]:
                ...
                print(f"  {ch['id']}: FAIL ({ch['field']} != expected)")
            else:
            ...
            if lf != rf:
                ...
                print(f"  {ch['id']}: FAIL ({ch['left_field']} != {ch['right_field']})")
            else:
                print(
    ...
    total = (
        len(vectors)
        + len(data["pair_invariants"])
        + len(data["chain_invariants"])
    )
    print(

/**
 * refund_receipt_v1 runner (Node.js / TypeScript reference impl).
 *
 * Validates 8 vectors + 5 pair invariants + 3 chain invariants in
 * refund_receipt_v1.json using @algovoi/substrate (>=0.3.0).
 *
 * Anchors to IETF draft-hopley-x402-refund-receipt-00 (Independent
 * Submission stream, Informational; AlgoVoi-authored). Composes with
...
const VECTORS_FILE = join(__dirname, "refund_receipt_v1.json");
...
function main() {
  ...
  console.log("refund_receipt_v1 runner (Node / @algovoi/substrate)");
  console.log(
  ...
  for (const v of data.vectors) {
    ...
    if (v.receipt) {
      console.log(
        `  ${v.vector_id}: PASS  result=${v.receipt.refund_result.padEnd(8)} ${label.slice(0, 14)}=${digest.slice(0, 16)}...`,
      );
    } else {
      console.log(
  ...
  for (const ch of data.chain_invariants) {
    if (ch.type === "field_equals") {
      ...
      if (actual !== ch.value) {
        ...
        console.log(`  ${ch.id}: FAIL (${ch.field} != expected)`);
      } else {
      ...
      if (lf !== rf) {
        ...
        console.log(`  ${ch.id}: FAIL (${ch.left_field} != ${ch.right_field})`);
      } else {
        console.log(

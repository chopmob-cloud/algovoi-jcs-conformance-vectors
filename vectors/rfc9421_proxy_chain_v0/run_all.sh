#!/usr/bin/env bash
# run_all.sh -- run every runner in this directory and tally PASS/FAIL.
#
...
run "Python (algovoi-rfc9421-verifier)" python runner_python.py
run "Node (@algovoi/rfc9421-verifier)" node runner_node.js
...
run ".NET 9 (NSec.Cryptography)" bash -c 'cd runner_dotnet && dotnet run -c Release --verbosity quiet'

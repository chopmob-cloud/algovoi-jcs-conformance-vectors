...
class Program
{
    ...
    static int Main()
    {
        ...
        string method = req.GetProperty("method").GetString()!; // rfc9421: preserve case
        ...
        foreach (var name in covered)
        {
            string val = name switch
            {
                ...
                _ => throw new Exception($"unexpected component for conformant set: {name}")
            };
            lines.Add($"\"{name}\": {val}");
        }
        // RFC 9421 §2.5: trailing @signature-params line (post-label portion of Signature-Input).
        string paramsRaw = siHeader.Substring(siHeader.IndexOf('=') + 1);

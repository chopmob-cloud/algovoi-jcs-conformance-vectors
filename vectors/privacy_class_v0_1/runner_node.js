#!/usr/bin/env node
/**
 * runner_node.js — canonicalize@3.0.0 (Erdtman) runner for x402 privacy_class attestation v0.
 *
 ...
 *   node --input-type=module runner_node.js privacy_class_v0.json
 *
...
function main() {
  if (process.argv.length < 3) {
    console.error('usage: node runner_node.js privacy_class_v0.json');
    process.exit(2);
  }
  ...
  for (const v of vectors) {
    const { bytes_b64, sha256 } = hashVector(v.attestation_body);
    computed[v.vector_id] = sha256;
    const expectedSha = v.expected_privacy_class_hash.replace(/^sha256:/, '');

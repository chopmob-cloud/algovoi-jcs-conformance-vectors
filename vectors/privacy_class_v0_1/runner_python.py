#!/usr/bin/env python
"""
runner_python.py — reference JCS runner for x402 privacy_class attestation v0 vectors.

Reads privacy_class_v0.json, recomputes JCS(RFC 8785)(attestation_body) + SHA-256 for
each vector, and verifies:

  1. recomputed SHA-256 == expected_privacy_class_hash
  2. base64(JCS bytes) == expected_jcs_bytes_b64
  3. pair expectations (same_hash_as / different_hash_from)

Usage:
    pip install rfc8785==0.1.4
    python runner_python.py privacy_class_v0.json
"""
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: runner_python.py privacy_class_v0.json", file=sys.stderr)
        return 2
    ...
    for v in vectors:
        bytes_b64, sha = hash_vector(v["attestation_body"])
        computed[v["vector_id"]] = sha
        expected_sha = v["expected_privacy_class_hash"].removeprefix("sha256:")

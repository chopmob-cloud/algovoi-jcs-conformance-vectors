"""
composite_trust_query_v1 runner (Python).

Validates 8 vectors + 7 pair invariants + 3 chain invariants in
composite_trust_query_v1.json using algovoi-substrate (>=0.3.0).

Anchors to IETF draft-hopley-x402-composite-trust-query-00. Composes
...
VECTORS_FILE = Path(__file__).parent / "composite_trust_query_v1.json"
...
def main() -> int:
    ...
    print("composite_trust_query_v1 runner (Python / algovoi-substrate)")
    print(
    ...
    for vid, v in vectors.items():
        payload = v.get("response") or v.get("row")
        ...
        label = "content_hash" if "response" in v else "row_content_hash"
        ...
        if "response" in v:
            r = v["response"]["trust_outcome"]
            print(f"  {vid}: PASS  outcome={r:22} {label[:14]}={digest[:16]}...")
        else:
            rn = v["row"]["row_number"]

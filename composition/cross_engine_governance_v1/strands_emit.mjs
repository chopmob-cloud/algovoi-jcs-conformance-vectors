...
// Strands-side governance integration (independent emitter, JS). Mirror of strands_emit.py.
export function emitFromStrands(event, grant, runtime) {
  const tu = event.tool_use;
  const agent_id = event.agent.agent_id;
  const agent_role = event.agent.role;
  const tool = tu.name;
  const tool_params = { ...tu.input };
  const g = grant;
  const decision = {
  ...
  return { decision, outcome, engine: "strands" };
}
